...
const EventCard = ({ eventId }: { eventId: Id<"events"> }) => {
  ...
  const renderQueuePosition = () => {
    ...
    //If user is next in line
    if (queuePosition.position === 2) {
      return(
      <div className="flex flex-col lg:flex-row items-center justify0between p-3 bg-amber-50 rounded-lg border border-amber-100">
        <div className="flex items-center">
          <CircleArrowRight className="w-5 h-5 text-amber-600 mr-2"/>
          <span className="text-amber700 font-medium">
    ...
    if (isEventOwner) {
      return (
        <div className="mt-4">
          ...
          ><PencilIcon className="w-5 h-5"/> Edit Event </button>
        </div>
      )
    }
    ...
    if (userTicket) {
      return (
        <div className="mt-4 flex items-center justify-between p-3 bg-green-50 rounded-lg border border-green-100">
          ...
          className="text-sm bg-green-700 hover:bg-green-700 text-white px-3 py-1.5 rounded-full font-medium shadow-sm transition-colors duration-200 flex items-center gap-1"
      ...
      <div className="flex flex-col items-end gap-2 ml-4">
        <span
         ...
         {availability.purchasedCount >= availability.totalTickets && (
          <span className="px-4 py-1.5 bg-red-50 text-red-700 font-semibold rounded-full text-sm">Dolf Out</span>
         )}
      ...
      <div className="mt-4 space-y">
        ...
        <div className="flex items-center text-gray-600">
          ...
          <span>
            ...
            {availability.totalTickets} Tickets Available
